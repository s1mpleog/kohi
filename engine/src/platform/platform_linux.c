...
// for logging and memory allocations things
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
KAPI b8 platform_startup(
    ...
    // retrive the connection from display
    state->connection = XGetXCBConnection(state->display);

    // error handling if connection is not success it returns KFATAL and false.
    if (xcb_connection_has_error(state->connection)) {
        KFATAL("Failed to connect to X server via XCB.");
        return FALSE;
    }

    // get the data from the x server
    ...
    // XCB_CW_BLACK_PIXEL = filling then window bg with a single color
    ...
    xcb_change_property(
        ...
        XCB_ATOM_WM_NAME,
        XCB_ATOM_STRING,
        8,  // data length should be viewed 8 bits at a time
        strlen(application_name),
        application_name);

    // tell the server to notify when the window manager
    // attempts to destry the window
